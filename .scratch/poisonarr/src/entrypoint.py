...
async def main():
    ...
    # Set up model config callback for per-role model management
    def model_config_callback(action: str, role: str, model: str):
        """Handle model config actions from the UI."""
        if action == "get_all":
            return config.models.to_dict()
        elif action == "get_role" and role:
            role_config = config.models.get_role(role)
            return role_config.to_dict() if role_config else None
        elif action == "set_role" and role and model:
            success = config.models.set_model(role, model)
            logger.info(f"Set {role} model to {model}: {'success' if success else 'failed'}")
            return {"success": success}
        elif action == "reset_role" and role:
            role_config = config.models.get_role(role)
            if role_config:
                role_config.reset_to_default()
                logger.info(f"Reset {role} model to default: {role_config.default_model}")
            return {"success": True}
        elif action == "get_metrics":
            return config.models.get_metrics_summary()
        elif action == "record_call" and role:
            # model parameter is actually time_ms in this case
            role_config = config.models.get_role(role)
            if role_config and model:
                role_config.record_call(float(model))
            return {"success": True}
        return None

    ui_server.set_model_config_callback(model_config_callback)
